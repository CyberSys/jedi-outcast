...
** GL_BindMultitexture
*/
void GL_BindMultitexture( image_t *image0, uint env0, image_t *image1, uint env1 ) {
	int		texnum0, texnum1;
...
void RB_BeginDrawingView (void) {
	...
	float fast_sky_color[4] = { 0.8f, 0.7f, 0.4f, 1 };  // FIXME: get color of sky
	//qglClear( clearBits );
...
const void	*RB_DrawBuffer( const void *data ) {
	...
	else if (tr.world && tr.world->globalFog != -1)
	{
		...
		color[0] = ( (byte *)&i )[0] / 255.0;
		color[1] = ( (byte *)&i )[1] / 255.0;
		color[2] = ( (byte *)&i )[2] / 255.0; 
		color[3] = 1.0f;
        vk_clear_attachments(false, true, color);
